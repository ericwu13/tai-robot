...
- **Verdict:** Reject as a migration target. **But** the underlying idea — a dark `ttk.Style` theme — is the correct Phase 1 quick win *without* CustomTkinter: a single `ttk.Style` pass (e.g. the sun-valley/`sv-ttk` theme or a hand-rolled clam-based style) plus replacing the raw-`tk`-for-color widgets with styled ttk covers ~80% of the visual gap at ~5% of the cost.
