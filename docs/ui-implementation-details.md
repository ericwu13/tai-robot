...
- **What changes** (exports):
  ...
  - **Revised after PR #136** (a recolored `clam` looked dated and rendered blurry on scaled displays): `init_theme(root, *, minimal=False)` creates a real ttk theme, `tairobot` (`style.theme_create(..., parent="default")`), whose elements are 9-slice images rasterized at runtime by `src/ui/raster.py` at the display's DPI scale — rounded buttons (`TButton` / `Accent.` / `Success.` / `Danger.` / `Ghost.`), fields, underline notebook tabs, arrowless pill scrollbars (+ `Hidden.*` variants for auto-hide), check/radio glyphs, `Card.TFrame` / `Well.TFrame` surfaces, and `Card.*` / `Bar.*` surface variants that only change the corner fill. `minimal=True` (headless) creates nothing: dotted style names fall back to stock layouts. Also exports `enable_dpi_awareness()` (call before COM init and `tk.Tk()`), `S(*px)` (logical→device pixels; every pixel metric in UI code goes through it), `apply_window_theme(win, caption=)` (dark caption + exact caption color via DWM) and `set_app_icon`. Two traps recorded in the code: ttk *tiles* the center slice of an image element, so sources must have wide centers (`raster.stretch_center`) or a large Treeview effectively hangs; and a bare `*Font` option-database entry lands on every ttk widget's `-font` and silently defeats the style fonts.
  - *(original, superseded)* `init_theme(root) -> ttk.Style` — `style.theme_use("clam")`; configures `TFrame/TLabel/TButton/TEntry/TCombobox/TNotebook/Treeview/TLabelframe/TPanedwindow` against `PALETTE`; defines named styles `Card.TFrame`, `CardValue.TLabel`, `Status.Ok.TLabel`, `Status.Warn.TLabel`, `Status.Err.TLabel`, `Dim.TLabel`; sets `root.configure(bg=PALETTE["bg"])` and Tk scaling (`root.tk.call('tk', 'scaling', …)` from `winfo_fpixels('1i')/72`).
...
### 1.2b `src/ui/raster.py`

- **Action**: CREATE
- **Why**: the theme needs anti-aliased rounded chrome at arbitrary DPI scales without shipping bitmap assets or adding Pillow to the PyInstaller bundle.
- **What changes** (exports): `Image` (float RGBA canvas with signed-distance-field primitives: `rounded_rect`, `bordered_rect`, `circle`, `polyline`), `encode_png` (zlib + struct, no dependency), `stretch_center` (widen a 9-slice source's center by byte replication), `solid_rows` (flat bands for hairlines), `mix` / `hex_to_rgb`. No Tk import — fully unit-testable without a display.
- **Dependencies**: precedes `theme.py`; listed in `tai_backtest.spec` hiddenimports.
...
- **What changes** (exports):
  ...
  - **Added in the smooth-UI pass**: `FlowFrame` (wrapping left-to-right layout positioned with `place` — replaces the hand-rolled `_reflow_toolbar`; a grid shares column widths across wrapped rows and tears them apart), `StatCard` (caption / large value / sub, optional sign-driven tint), `scrolled_tree`, `autohide_scrollbar` (swaps to a same-footprint `Hidden.*` style instead of unmapping — no re-wrap oscillation), `enable_row_hover`, `place_dialog` (natural-size, parent-centered dialogs; fixed pixel geometries clip at high DPI).
