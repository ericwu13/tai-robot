...
class LiveRunner:
    ...
    def __init__(
        ...
        self.csv_logger = CsvLogger(log_dir, symbol)
    ...
    def _process_aggregated_bar(self, bar: Bar) -> None:
        ...
        if idx > 0:
            ...
            self._check_for_trade_close(bar)
        ...
        # Fill entry orders at bar close
        self.broker.on_bar_close(idx, bar.close)
        self._check_for_entry_fill(bar)

        self._emit("on_bar", bar)

    def _check_for_trade_close(self, bar: Bar) -> None:
        """Check if a trade was just closed by check_exits."""
        if self.broker.trades and self.broker.trades[-1].exit_bar_index == self._bar_index:
    ...
    def _check_for_entry_fill(self, bar: Bar) -> None:
        """Check if an entry was just filled."""
        if self.broker.position_size > 0 and self.broker.entry_bar_index == self._bar_index:
            self._log_decision(
